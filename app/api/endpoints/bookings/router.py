...
async def book_room(room_id: Booking_room, session: AsyncSession = Depends(get_async_session), user=Depends(get_current_user)):
    try:
        ...
        return (f"{user.email} Вы успешно забронировали отель с {room_id.date_from} по {room_id.date_to}")
